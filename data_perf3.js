...

{
  ...
  choices: ["Tournament rounds: start many configs on a SMALL budget (data or iterations), keep the best fraction, multiply the budget, repeat — losers cost a sliver, finalists get full resources", "Train everything but stop the wall-clock earlier", "Sample fewer configurations at random", "Use a smaller model as a proxy for all configs", "Cache and reuse gradients across configs"],
...

{
  ...
  choices: ["Fitting a cheap surrogate model of 'score vs hyperparameters' from past trials, then picking the next trial where the surrogate promises the best mix of high mean and high uncertainty — explore/exploit steering", "Averaging the ten best configs found so far", "Following the gradient of the validation score", "Halving the search space after every trial", "Rerunning the best trial with new seeds"],
...

{
  ...
  choices: ["Treat every score as a distribution: multiple seeds/folds, report mean ± spread, and only believe differences that exceed the spread — ideally tested PAIRED on the same folds", "Always trust the first run; seeds average out", "Increase both models' epochs until the gap stabilises", "Report the best seed for each model — fairest to both", "Differences under 1% are always real if CV was used"],
...

{
  ...
  choices: ["A spike's excellence is fragile — likely part validation-luck, and gone the moment data drifts — while a plateau means performance is INSENSITIVE to the exact values: robustness you can ship", "The plateau trains faster", "Spikes indicate data leakage always", "The plateau uses smaller parameter values, which are safer", "Practitioners distrust all grid searches"],
...

{
  ...
  choices: ["A hierarchy of leverage: data quality > features > model/loss choice > hyperparameters — tune LAST, time-box it, and let diminishing returns tell you when to move up the stack", "Hyperparameters first — they're cheapest to try", "All four levers pay equally per hour", "Data work is junior work; models are where experts win", "Tuning compounds, so it eventually beats data work"],
