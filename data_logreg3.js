...

{
  ...
  choices: ["Multicollinearity: near-duplicates make individual weights unidentifiable — only their SUM is pinned down, so huge opposite weights are 'free' — while predictions remain stable", "A numerical instability: with two near-identical columns the matrix the solver inverts is nearly singular, so the coefficients come out essentially random and the model can't be trusted", "Target leakage: one of the two features is really a copy of the label, so training rewards a giant weight on it and the huge opposite weight on its twin is the model exploiting that leak", "Classic overfitting: the redundant column gives the model more capacity than the data supports, so it memorises noise and those wild coefficients will wreck its accuracy on fresh data", "The two features sit on different scales, so their weights compensate with large opposite values; standardising both columns to unit variance will shrink them straight back to sensible sizes"],
...

{
  ...
  choices: ["An INTERACTION term (medication × age): plain logistic regression is additive in its features and cannot express 'the effect of X depends on Y' unless you hand it the product", "A squared age term (age-squared): the drug's reversal with age is a curved relationship, and adding age-squared lets its single medication weight bend to match that curve across all ages", "Simply far more training data: the young-benefit and old-harm cases are rare, so with enough of both the shared medication weight will eventually resolve the apparent conflict on its own", "Two separate models, one fit on young patients and one on the elderly, since a single additive model can never carry two opposite medication effects at once, so split by age and train each", "Turning off sklearn's default L2 penalty: the regularisation is shrinking the medication weight toward its average effect, so removing the penalty lets the true age-varying effect resurface"],
...

{
  ...
  choices: ["lbfgs = default for L2 on medium data; saga = the one for L1/elastic-net on BIG data; liblinear = small binary problems; and scale your features — unscaled data is the #1 cause of non-convergence", "newton-cg = the required solver for L1 at any size; liblinear = fastest on millions of rows; lbfgs = tiny problems only; and a convergence warning means you must always drop the learning rate", "All four solvers are interchangeable on a convex loss, so pick any; the 'failed to converge' warning only means max_iter is too low, so raise it and the solver you chose never matters", "saga = L2 only; liblinear = the sole solver supporting elastic-net; newton-cg = the big-data choice; and non-convergence almost always means your classes just aren't separable enough to fit", "liblinear = the modern default for every case; saga is deprecated now; and convergence warnings mean the loss has gone non-convex at scale, so you should simply switch to a tree-based model instead"],
...

{
  ...
  choices: ["sklearn regularises BY DEFAULT (C=1.0 shrinks every coefficient) and provides no standard errors — for inference, disable the penalty or use statsmodels", "sklearn fits by unpenalised maximum likelihood so the estimates are unbiased; the only catch is exponentiating coef_ yourself to turn the log-odds into reportable odds ratios", "The coefficients are fine, but sklearn's p-values use a normal approximation invalid for logistic regression, so you must bootstrap every confidence interval before publishing", "sklearn standardises the features internally, so coefficients come out on the z-score scale and must be back-transformed to raw units before any confidence interval makes sense", "The penalty and errors are fine; the real trap is that sklearn fits one-vs-rest by default, giving separate coefficient sets that can't be merged into one inferential report"],
...

{
  ...
  choices: ["Multinomial optimises one joint softmax whose probabilities are coherent by construction; OvR trains independent yes/no models whose scores can disagree and need renormalising — usually similar accuracy, cleaner probabilities from multinomial", "The two are mathematically identical in every case; the only real difference is speed, since OvR fits its four binary models in parallel while multinomial must solve one larger optimisation in a single pass", "Multinomial trains four separate binary classifiers and majority-votes them while OvR fits the one joint model, so it's OvR that gives coherent probabilities and multinomial whose outputs need renormalising", "OvR is reliably more accurate because each binary model can specialise on its own class, whereas multinomial forces one shared weight set to compromise across every class at once and blurs the boundaries between them, which is why it tends to win in practice", "They agree for two classes, but beyond that only OvR is valid, since softmax can't normalise three or more competing scores without some of the probabilities drifting above 1 and then needing to be clipped"],
...

{
  ...
  choices: ["It blends L1 + L2: the L2 part spreads weight across the correlated group (keeping or dropping it together), while the L1 part still zeroes genuinely useless features", "It runs plain lasso on many bootstrap samples and keeps only the features chosen in nearly every run, so the arbitrary single-witness pick is averaged into a stable, reproducible selection", "It swaps the absolute-value penalty for a smooth approximation so the optimiser no longer lands exactly on zero, which removes the randomness in which correlated feature happens to survive", "It first clusters the correlated features into groups and fits one shared weight per group, then applies ordinary L1 across the groups so each cluster survives or dies together as a unit", "It keeps pure L1 but raises the penalty until all five correlated features drop to zero together, trading a little accuracy for a selection that is at least stable and repeatable across retrains"],
