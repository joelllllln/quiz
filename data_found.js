...

{
  ...
  choices: ["Sold price is the LABEL — it's what we predict, and it's exactly the column we won't have for new houses", "Square footage — it's the largest number", "Age — time flows differently", "Location score — it's subjective", "None; all five columns are interchangeable"],
...

{
  ...
  choices: ["Whether labelled answers exist to learn from: spam and prices are supervised; the other three must find structure without answers — unsupervised", "Whether the data is text or numbers", "Whether the output is needed fast", "Dataset size", "Whether a neural network is required"],
...

{
  ...
  choices: ["The first predicts a CATEGORY (classification), the second a NUMBER (regression) — the choice changes the algorithms, the outputs and the error measures", "The first is easier", "The second needs more data", "The first is unsupervised", "No distinction — both are predictions"],
...

{
  ...
  choices: ["The model's internal numbers are repeatedly nudged in whatever direction shrinks its measured error on the training examples", "The data is sorted and indexed for fast lookup", "The computer memorises every row verbatim", "A human tunes the answers by hand", "The features are converted to text"],
...

{
  ...
  choices: ["Because scoring on training rows measures memory, not skill — only never-seen rows preview how the model will do on the future", "Because training data gets corrupted during fitting", "Because two scores are better than one", "To make training faster", "It's tradition, with no practical effect"],
...

{
  ...
  choices: ["Underfitting and overfitting — the best model sits between them, at the peak of performance on HELD-OUT data", "Bias and luck — the best model has neither", "Lag and drift — the best model updates daily", "Both are kinds of leakage", "Underfitting and overfitting — the best model is always the most flexible one"],
...

{
  ...
  choices: ["Hyperparameters (your settings) vs parameters (learned values) — you tune the first via validation experiments; training handles the second", "Constants vs variables — a programming convention", "Inputs vs outputs", "Metadata vs data", "There is no distinction — all numbers in a model are equal"],
...

{
  ...
  choices: ["New cases are almost never identical to old ones — a model must GENERALISE: capture the pattern so it can answer cases it never saw", "Lookup tables are too slow", "It collapses only if the table is unsorted", "Real models memorise MORE cases", "It doesn't collapse — memorisation is the goal"],
...

{
  ...
  choices: ["Salary's huge numeric range drowns age in every calculation — standardise features onto comparable scales before fitting", "Nothing — algorithms are unit-aware", "Age must be converted to months", "Salary must be paid in dollars", "Only text features need preparation"],
...

{
  ...
  choices: ["98% — beating the celebrated model. Every result must be compared against the best 'dumb' baseline before it means anything", "50% — random guessing", "2% — it misses all the fraud", "0% — it makes no predictions", "97.8% — the same by definition"],
...

{
  ...
  choices: ["Bias (rigidity: wrong on average) and variance (instability: different every retrain) — reducing one usually inflates the other", "Precision and recall", "Noise and signal", "Leakage and drift", "Underflow and overflow"],
...

{
  ...
  choices: ["odds = p/(1−p): 75% ⇔ 3:1 — three churns per one stay; the same belief in ratio form", "Odds are always double the probability", "75% means odds of 75:1", "They cannot be converted without more data", "Odds apply to horses, probability to models"],
