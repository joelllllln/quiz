...

{
  ...
  choices: ["K-means = GMM with spherical, equal-size, equal-weight components and HARD assignments — lifting them buys soft membership probabilities, elliptical clusters, and different cluster sizes", "K-means is a GMM with exactly two components", "GMMs cannot do clustering; the link is folklore", "K-means adds regularisation that GMMs lack", "The two methods share nothing but the letter k"],
...

{
  ...
  choices: ["Bad initial centroid draws that trap the algorithm in visibly wrong solutions (two centroids sharing one blob, one straddling two) — run many seeded starts, keep the lowest-inertia finish", "Slow convergence on large datasets", "The elbow method failing", "Numerical overflow in distance computation", "Nothing — k-means++ already guarantees the global optimum"],
...

{
  ...
  choices: ["Boundaries between clusters are always STRAIGHT lines equidistant between centroids — so k-means cannot respect elongated/curved cluster shapes, and it tends to split big clusters to balance spatial extent", "Slow convergence and high memory use", "Sensitivity to duplicate rows and outliers only", "It cannot handle more than 10 clusters", "Voronoi cells only exist in two dimensions"],
...

{
  ...
  choices: ["Agreement between two clusterings over point PAIRS (together-vs-apart), renamed labels ignored — 'adjusted' subtracts the agreement random labelings would get by chance, so 0 = chance, 1 = identical groupings", "The distance between the two runs' centroids", "Whether both runs achieved equal inertia", "The percentage of identical label integers", "It requires ground-truth classes to compute"],
...

{
  ...
  choices: ["Cluster the unlabelled mass, then let each cluster's few labels speak for the whole cluster (or use distances-to-centroids as features) — unlabelled data reveals structure that makes 50 labels go a hundred times further", "The clusters replace the need for any labels", "Label propagation requires deep networks", "It only works when clusters equal classes exactly", "Semi-supervised learning is another name for data augmentation"],
