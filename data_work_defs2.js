...
(function () {
  ...
  def("wcrypto", 2,
    ...
    "Anyone can join and validate on a public chain; a permissioned one restricts participation to approved parties.",
    ["Public chains are free to use while permissioned chains charge a subscription fee for every transaction made.",
     ...
     "Permissioned chains hold only payment data while public chains can store any kind of application data.",
  ...
  def("wcrypto", 2,
    ...
    "A system built on top of a blockchain that processes transactions off the main chain, settling back to it for security.",
  ...
  def("wcrypto", 2,
    ...
    "Locking up tokens as collateral to help validate the network, earning rewards but risking penalties.",
  ...
  def("wcrypto", 2,
    ...
    "An organisation coordinated by smart contracts and token-holder votes rather than directors and employees.",
  ...
  def("wcrypto", 2,
    ...
    "Infrastructure that moves value between blockchains, typically by locking assets on one and issuing equivalents on another.",
  ...
  def("wcrypto", 1,
    ...
    "Rules requiring crypto marketing to UK consumers to be fair, clear, risk-warned and issued or approved by authorised firms.",
  ...
  def("wcrypto", 2,
    ...
    "The requirement that transfers of cryptoassets carry identifying information about sender and recipient between firms.",
  ...
  def("wpay", 2,
    ...
    "Clearing works out who owes what; settlement is the actual transfer of funds that discharges those obligations.",
    ["Clearing removes fraudulent transactions while settlement processes the legitimate ones that remain.",
     ...
     "They are two names for the same step, used respectively by UK and US payment systems."],
  ...
  def("wpay", 2,
    ...
    "The name-checking service that warns you when the recipient name doesn't match the account you're paying.",
    ["The receipt a bank must issue within one hour of any Faster Payments transfer completing.",
     ...
     "The process by which an employer confirms a new employee's bank details with their bank."],
  ...
  def("wpay", 2,
    ...
    "Paying directly from one bank account to another — often via open banking — bypassing the card networks entirely.",
    ["A transfer between two accounts held by the same customer at the same retail bank.",
     ...
     "Any payment where the payer and payee both bank with the same institution."],
  ...
  def("wpay", 2,
    ...
    "The card-scheme process for reversing a disputed transaction, pulling funds back from the merchant.",
    ["The monthly fee an acquirer charges merchants for each card terminal they operate in store.",
     ...
     "The refund a merchant offers voluntarily when goods are returned within thirty days."],
    "Chargeback",
  ...
  def("wpay", 1,
    ...
    "The requirement that e-money and payment firms keep customer funds separate and protected, not used as their own.",
  ...
  def("wpay", 2,
    ...
    "Licensed banks renting out their regulated capabilities — accounts, payments, cards — to other brands via APIs.",
    ["The outsourcing of a bank's IT helpdesk and infrastructure maintenance to cloud providers.",
     ...
     "A comparison service that helps consumers switch their current account between banks."],
  ...
  def("wpay", 2,
    ...
    "Financial products woven invisibly into non-financial journeys — insurance at checkout, credit inside a ride app.",
    ["The mandatory financial-education content that banking apps must display to younger customers.",
     ...
     "Savings accounts whose interest is fixed for the entire life of the product."],
    "Embedded finance",
  ...
  def("wpay", 2,
    "What is ISO 20022?",
    "The modern global messaging standard for payments, carrying rich structured data with every transaction.",
    ["The security certification that payment terminals must hold before accepting contactless cards.",
     ...
     "The numbering scheme that assigns a unique identifier to every bank branch worldwide."],
    "ISO 20022",
  ...
  def("wpay", 1,
    ...
    "The PSR regulates payment systems and their access/competition; the FCA regulates the firms and their conduct.",
    ["The PSR is the FCA's internal payments division, operating under the same board and chief executive.",
     ...
     "The PSR handles consumer complaints about payments while the FCA sets the underlying rules.",
  ...
  def("wpay", 2,
    ...
    "An app holding tokenised versions of your cards, authorising payments with device authentication.",
  ...
  def("waws", 2,
    ...
    "AWS's monitoring service — metrics, logs and alarms for everything running in your account.",
  ...
  def("waws", 2,
    "What is Amazon RDS?",
    "AWS's managed relational database service — it runs PostgreSQL, MySQL and friends, handling patching and backups.",
    ["A rapid deployment system that pushes application code to thousands of servers at once.",
     ...
     "A caching layer that accelerates repeated reads from S3 object storage."],
  ...
  def("waws", 2,
    ...
    "The traffic distributor that spreads incoming requests across multiple instances, routing around unhealthy ones.",
  ...
  def("waws", 2,
    ...
    "A lightweight package of an app plus its dependencies, sharing the host's OS — many can run on one instance.",
  ...
  def("waws", 2,
    ...
    "The orchestrator that runs containers across a fleet — scheduling, scaling and healing them; EKS is AWS's managed version.",
    ["A programming language created by Google for writing cloud-native web applications.",
     ...
     "A NoSQL database optimised for storing container images and their metadata."],
    "Kubernetes / EKS",
  ...
  def("waws", 2,
    ...
    "Defining cloud resources in version-controlled files, so environments are built by running code, not clicking consoles.",
  ...
  def("waws", 2,
    ...
    "Glue catalogues and transforms data in S3; Athena runs SQL queries directly over those S3 files.",
    ["Glue synchronises files between employee laptops while Athena archives them for legal holds.",
     ...
     "Glue encrypts the data lake while Athena manages the keys used for that encryption."],
  ...
  def("waws", 2,
    ...
    "AWS's managed service for using foundation models — Claude, Llama and others — via API, without hosting them yourself.",
    ["The bottom storage tier where S3 keeps objects that haven't been accessed in years.",
     ...
     "A database of pre-trained weights that customers can download for offline use."],
    "Bedrock",
  ...
  def("waws", 1,
    ...
    "AWS secures the cloud itself; the customer secures what they build in it — configuration, access, data.",
    ["AWS and the customer split the cost of any security breach equally between them by contract.",
     ...
     "The customer secures the hardware while AWS manages all passwords and permissions."],
  ...
  def("waws", 1,
    ...
    "Deleting resources you're finished with — the discipline that stops forgotten instances burning money.",
  ...
  def("wai", 1,
    ...
    "A neural network trained on vast text corpora to predict tokens, which lets it generate and reason over language.",
  ...
  def("wai", 1,
    ...
    "AI that produces new content — text, images, code, audio — rather than only classifying or scoring inputs.",
  ...
  def("wai", 1,
    ...
    "A very large model trained on broad data that many different applications then build on, often via fine-tuning.",
    ["The first model a firm ever deploys, which all later models must remain compatible with.",
     "A model whose weights are permanently frozen by regulation once it enters service.",
  ...
  def("wai", 1,
    ...
    "Confidently stated output that is factually wrong or entirely invented — fluent, plausible, and false.",
  ...
  def("wai", 1,
    ...
    "Fetching relevant documents first and having the LLM answer from them — grounding output in real sources.",
    ["Training a model repeatedly on its own previous outputs to amplify its capabilities.",
     ...
     "The compression technique that shrinks large models to run on ordinary laptops.",
  ...
  def("wai", 2,
    ...
    "Fine-tuning updates the model's weights on your examples; prompting just steers the frozen model with instructions.",
  ...
  def("wai", 1,
    ...
    "The governance discipline for models: validation, monitoring, documentation and clear ownership of model failures.",
  ...
  def("wai", 1,
    ...
    "Systematically worse outcomes for particular groups, learned from data or design — regardless of intent.",
  ...
  def("wai", 2,
    ...
    "Being able to give meaningful reasons for a model's decisions — to firms, supervisors and affected customers.",
    ["Publishing a model's full source code and weights on the firm's public website.",
     ...
     "A marketing standard for describing AI products in plain-English brochures.",
  ...
  def("wai", 2,
    ...
    "Artificially generated data that mimics real data's statistical properties without containing real records.",
  ...
  def("wai", 2,
    ...
    "Innovation tools: a testing platform with synthetic data and APIs, and time-boxed collaborative problem-solving events.",
  ...
  def("wai", 2,
    ...
    "Watching whether live inputs or performance shift away from training conditions, so decay is caught early.",
