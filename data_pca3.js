...

{
  ...
  choices: ["All three — maximum-variance directions, minimum reconstruction error, and the top singular vectors are the SAME thing, which is why SVD is how PCA is actually computed", "Only the variance definition is correct", "They describe three unrelated algorithms", "SVD is an approximation to true PCA", "Reconstruction error applies only to autoencoders"],
...

{
  ...
  choices: ["Project the data onto a few more than 10 RANDOM directions to cheaply capture the dominant subspace, then do an exact small SVD inside that captured subspace — accuracy for the top components with almost no cost", "It samples 10 rows and runs exact PCA", "It rounds the matrix to integers first", "It only works when the matrix is sparse", "It computes all 20,000 components then keeps 10"],
...

{
  ...
  choices: ["Forcing all factors to be NON-NEGATIVE yields PARTS-BASED, additive components (a face = nose + eyes + mouth; a document = topics) — sparse and interpretable, unlike PCA's cancelling ± mixtures", "It makes the components orthogonal like PCA", "It guarantees a unique global optimum", "It only works on image data", "It is PCA with the mean added back"],
...

{
  ...
  choices: ["PCA CENTERS the data (subtracts the mean), which fills every zero and destroys sparsity → a dense 100k×100k blow-up; TruncatedSVD skips centering, runs on the sparse matrix directly (this is 'LSA' for text)", "PCA requires fewer than 1000 columns by design", "The data must be shuffled first", "TruncatedSVD is just PCA with a different name", "Sparse matrices cannot be reduced at all"],
...

{
  ...
  choices: ["PCA is UNSUPERVISED — it ranks directions by variance, which needn't align with what predicts the LABEL; a low-variance direction can carry the signal, and PCA may discard it", "PCA always improves accuracy; the pipeline has a bug", "10 components is always too few", "Standardizing before PCA is the mistake", "The classifier needs the raw feature names"],
