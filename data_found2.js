...

{
  ...
  choices: ["Data leakage — the test rows influenced the preprocessing, so 'unseen' data had already left fingerprints on the model", "Overfitting — the model grew flexible enough to trace the test rows exactly, so its memorised noise inflated the reported score", "Sampling bias — the split happened to drop the easiest rows into the test set, so the score reflects luck more than real skill", "Double scaling — the features were standardised twice, once before and once after the split, shrinking their spread too far", "Target leakage — a column that secretly encodes the answer slipped into the features, so the model just read the labels off it"],
...

{
  ...
  choices: ["Class imbalance: the rare class barely influences training — fix with class weights, resampling, and metrics that watch the minority", "Underfitting: the model is far too rigid to separate the two classes — fix with a much deeper network and many more training iterations", "Label noise: most of the churn flags are simply mislabelled — fix by relabelling the minority rows and dropping ambiguous ones", "Feature collapse: the churn signal hides inside one weak column — fix by engineering the ratios and interactions that expose it", "Threshold drift: the default 0.5 cutoff buries the churners — fix by lowering the cutoff and leaving the training data untouched"],
...

{
  ...
  choices: ["Correlation isn't causation — the model found that the two go TOGETHER (likely via a hidden common cause), not that one produces the other", "Reverse causation — churning quietly makes people stop buying ice cream, so acting on that arrow backwards can never actually lift retention at all", "Selection bias — only the most loyal customers were sampled for the ice-cream analysis, so the pattern won't hold on the full base", "Simpson's paradox — the pooled trend flips inside every subgroup, so the free ice cream would actually push churn upward instead", "Overfitting — the model merely memorised the ice-cream buyers it saw, so the retention link will vanish on next quarter's data"],
...

{
  ...
  choices: ["The learning curve — score vs training-set size: still-rising curves say buy data; converged-but-low curves say improve the model or features", "The validation curve — score vs model complexity: a rising branch says add capacity, a falling one says the data is the bottleneck", "The ROC curve — true vs false positive rate: a curve near the diagonal says buy data, a bowed one says the features already suffice", "The residual plot — errors vs predicted value: a widening fan says gather more rows, a flat band says the model family itself is the real limit", "The loss curve — error vs training epoch: a still-falling line says buy data, an early plateau says switch to a richer model instead"],
...

{
  ...
  choices: ["Dropping rows burns data and biases the sample; imputation keeps rows but invents values — impute thoughtfully AND add a 'was missing' indicator, since missingness itself is often informative", "Dropping rows is the safest default whenever data is plentiful, while imputation quietly injects bias — so prefer deletion above a 5% missing threshold and median-fill only the small remainder that is genuinely left behind", "Both options are fine as long as they stay consistent; the rule is to compute one fill value on the full dataset and apply that identical number to the train and test rows alike so the two never diverge", "Deletion and median-filling both destroy signal, so the mature move is to model every missing entry from the other columns and never flag it, since a visible was-missing column would only leak the target", "Imputation always beats deletion, so fill each blank with the column mean and, because the fact of absence is just random noise, discard any indicator marking where the values had been missing"],
...

{
  ...
  choices: ["Feature engineering usually moves the needle more than model swapping — a model can only use the information you make reachable for it", "Bigger ensembles usually beat single models — B simply stacked far more learners than A did and averaged their votes into a stronger score", "Simpler models usually generalise better — B's plain logistic regression overfit less than A's forest, so it kept its lead on new data", "Hyperparameter tuning usually beats architecture changes — B spent the sprint tuning logistic regression's penalty instead of swapping models", "Domain features tend to leak the target — B's crafted columns quietly encoded churn itself, which is exactly why its score jumped so far"],
