...

{
  ...
  choices: ["NB's 'parameters' are just counts, so new examples are absorbed by incrementing counters — partial_fit updates the model in O(new data) without revisiting anything", "NB keeps every past email in memory and simply re-sorts the whole archive each night, which stays fast because sorting is cheap", "It isn't suited at all - like every classifier, NB must be fully retrained from scratch whenever a fresh batch of emails arrives", "A tuned learning-rate schedule lets NB replay a sample of the old emails alongside every new batch, gradually nudging its stored weights toward the drifting stream", "Only gradient-based neural networks can truly stream; NB only fakes it by retraining on a sliding window of the most recent emails"],
