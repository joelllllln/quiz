...

{
  ...
  choices: ["The product underflows to exactly 0.0 in floating point — so implementations SUM LOG-probabilities, which is mathematically identical and numerically safe", "Multiplying that many factors is simply too slow - so implementations sample 50 representative words and multiply only those", "Any probability below the float floor rounds UP to the smallest positive value - so implementations cap the running product at 10^-308 and compare the caps", "The rounding error accumulates and randomises the result - so implementations add a small constant to every factor before multiplying them", "Repeated multiplication is undefined past a few hundred terms - so implementations split the email into short chunks and average the chunk products"],
...

{
  ...
  choices: ["Equal variances across classes → linear boundary; UNEQUAL variances → quadratic (the log of a bell-curve is a parabola, and unequal parabolas differ by a curve)", "It is always linear by definition - GaussianNB's diagonal-covariance assumption forces every single decision boundary to be a straight hyperplane, whatever the variances", "Equal MEANS across classes -> linear boundary; UNEQUAL means -> quadratic (it is the class centres, not their spreads, that set the curve)", "The boundary curves only when the class PRIORS differ; with equal priors GaussianNB always draws a straight-line split between them", "Any curved boundary needs a kernel; plain GaussianNB is strictly linear and must be paired with an RBF feature map before it can bend"],
...

{
  ...
  choices: ["NB's 'parameters' are just counts, so new examples are absorbed by incrementing counters — partial_fit updates the model in O(new data) without revisiting anything", "NB keeps every past email in memory and simply re-sorts the whole growing archive from scratch each night, which stays fast only because sorting is cheap", "It isn't really suited at all - like every other classifier, NB must be fully retrained from scratch whenever even one fresh batch of emails arrives", "A tuned learning-rate schedule lets NB replay a sample of the old emails alongside every new batch, gradually nudging its stored weights toward the drifting stream", "Only gradient-based neural networks can truly stream; NB merely fakes it by quietly retraining on a sliding window of the few thousand most recent emails"],
...

{
  ...
  choices: ["Exploit independence: score each feature GROUP with its matching variant (Multinomial/Bernoulli/Gaussian) and ADD their log-scores plus one log-prior — one coherent NB model", "Force every feature through GaussianNB by treating the counts and the yes/no flags as if they were continuous numbers, then fit and read a single bell curve for every one of the features", "Round the continuous amounts into integer, word-like tokens so that MultinomialNB alone can swallow all three feature types in one model", "Train a separate NB VARIANT for each class - Gaussian for fraud, Multinomial for legit - and then compare the two models' raw scores directly", "NB fundamentally cannot mix distributions, so you must keep only the single feature type that matters most and discard the other two blocks"],
...

{
  ...
  choices: ["Bernoulli explicitly multiplies in P(word ABSENT | class) for every vocabulary word not present — absence is evidence too, and in short texts absence is most of the signal", "Bernoulli counts every repeated word one extra time, so short bursty tweets full of one repeated term end up scoring far more strongly", "Bernoulli quietly trains on a substantially larger vocabulary than Multinomial does, and those extra rare words are what give it the edge on tweets", "Bernoulli up-weights the earliest words it encounters in each document, and on a short tweet that means almost every single word receives the boosted leading-position weight", "Multinomial simply cannot process documents shorter than about twenty words, so Bernoulli wins the short-text case essentially by default"],
...

{
  ...
  choices: ["NB contributes a powerful compressed summary of thousands of word features that LR could never estimate from small data — and LR learns how much to TRUST that summary, fixing NB's overconfidence", "Feeding NB's output back in effectively doubles the amount of training signal available, which hands the downstream logistic regression roughly twice as many effective examples to fit its weights on", "The two models happen to make opposite mistakes, so stacking them lets each one randomly cancel out the other's errors and the average lands closer to the truth", "Appending a log-odds column is universally beneficial - almost any monotonic transform of the labels, added back as a feature, reliably lifts a logistic regression's accuracy", "The hybrid only works because the final softmax layer renormalises NB's wildly extreme probabilities back into a smooth, well-behaved range before LR ever sees them"],
