...

{
  ...
  choices: ["High-cardinality features offer thousands of candidate cut points, so they win splits by sheer opportunity — impurity importance rewards splitting OPPORTUNITY, not real signal; permutation importance on held-out data is the fix", "Low-cardinality features starve the tree of cut points, so binary flags win splits by scarcity — impurity importance rewards RARITY of values, not real signal; drop-column testing on held-out data is the fix for this reversed cardinality bias", "Deeper trees over-credit whichever feature sits nearest the root, so the hash won purely by being tested first — the cure is simply to cap tree depth and re-read the importances afterward", "Impurity importances are computed before any normalisation, so a raw hash with large integer values inflates the totals — rescaling every feature to unit variance restores the true ranking", "The forest was grown with far too few trees to average out noise, so a random column topped the list by chance alone — adding several hundred more trees would demote the hash on its own"],
...

{
  ...
  choices: ["Shuffling one twin lets the model lean on the other, hiding shared signal — permute or drop correlated features as a GROUP to measure their joint contribution", "Permutation double-counts correlated pairs, so both twins absorb the full shared signal — divide each feature's raw score by its total number of correlated partners to correct it", "The forest hands all shared credit to whichever twin it happened to split on first — forcing the trees to try savings first would even out the two numbers", "Correlated features cancel inside the vote, so their real importance is the DIFFERENCE of the scores — report 0.29 for the pair instead of the misleading sum", "Shuffling severs the link the trees relied on, inflating income far above its true worth — apply the SAME permutation to both twins to keep their correlation intact"],
...

{
  ...
  choices: ["Unanimity is nearly impossible across 500 diverse trees, so averaged votes shrink toward the middle — under-confidence at the extremes; calibrate (isotonic/Platt) when downstream decisions consume the raw probabilities", "A few deep trees memorise every training case, so averaged votes spike toward 0 and 1 — over-confidence at the extremes; calibrate (isotonic/Platt) whenever downstream decisions consume the raw probabilities directly as inputs to a cost formula", "Majority voting discards the losing fraction outright, so probabilities snap to coarse steps like 0.2 or 0.8 — quantised output; refit the forest with more classes when the steps look too jagged", "Bootstrap sampling leaves each tree short of positive cases, so probabilities drift permanently below 50% — a downward bias; reweight the classes during training to recentre the reliability curve", "The trees average their raw impurity scores rather than votes, so outputs cluster near the base rate — flat output; switch to hard voting to spread the probabilities back out toward the extremes"],
...

{
  ...
  choices: ["Honest for large forests on i.i.d. rows; misleading with FEW TREES (each row judged by a tiny, noisy sub-committee) and with GROUPED/temporal rows (a patient's other rows sit in training, so OOB inherits the leak)", "Honest for small forests on any data; misleading with MANY TREES (each row judged by nearly the whole forest, so OOB and training accuracy quietly converge) and with perfectly balanced classes spread evenly across every fold", "Honest whenever classes are balanced; misleading with IMBALANCED targets (the rare class is seldom left out, so OOB flatters it) and with UNSCALED features (the bootstrap distorts every distance)", "Honest for shallow trees only; misleading with DEEP TREES (each memorises its bootstrap, so left-out rows score near zero) and with HIGH max_features (correlated trees repeat the same OOB errors)", "Honest for regression forests; misleading for CLASSIFICATION (ties break arbitrarily and inflate OOB) and for LARGE datasets (the 37% held-out fraction shrinks, so the estimate grows biased)"],
...

{
  ...
  choices: ["Random splits corner an isolated outlier in a few cuts, while points deep in dense crowds need many — short average path = easy to isolate = anomalous", "Random splits reach a lone outlier only after very many cuts, while dense-crowd points drop out almost immediately — long average path = hard to isolate = anomalous", "Each tree learns a boundary around the normal class, and depth counts how far a point sits beyond it — deep paths are what flag the anomalies", "Depth records how many neighbours share a point's final leaf, and outliers land in crowded leaves — shallow crowded paths mark the anomalies", "The forest averages each point's distance to the cluster centroids, and depth encodes that distance — the farthest points score as anomalies"],
...

{
  ...
  choices: ["Each leaf's full SET of training targets (not just their mean) — pooling those across trees gives a whole predictive distribution, from which any quantile is read off", "Each leaf's variance alongside its mean — combining those across the trees fits a Gaussian per prediction, from which any quantile is then looked up from the standard normal table", "The residuals of every training row — resampling those across the trees builds a bootstrap of the point forecast, whose spread is read as the quantiles", "Each tree's single prediction — collecting all 500 of those numbers forms an empirical spread whose percentiles are reported as the quantile estimates", "The out-of-bag error at each leaf — accumulating those across the forest calibrates one global band that is pasted symmetrically around the mean"],
