...

{
  ...
  choices: ["Both peak at a 50/50 mix and hit zero when pure — they pick near-identical splits, and gini skips the logarithm so it's faster", "Entropy finds deeper trees, gini shallower ones", "Gini works only for two classes", "Entropy is for regression, gini for classification", "Gini peaks at pure nodes"],
...

{
  ...
  choices: ["It forbids leaves smaller than N samples, so the tree can't memorise single points — validation peaks at a middle value", "It caps the tree's depth directly — validation peaks at 1", "It limits how many features are used — validation climbs forever", "It removes N outliers before training", "It sets the minimum accuracy per leaf"],
...

{
  ...
  choices: ["Snips branches whose accuracy gain doesn't justify their added leaves — bigger alpha prunes harder", "Deletes the deepest level only", "Removes features with low importance", "Restarts training with fewer samples", "Merges the two smallest classes"],
...

{
  ...
  choices: ["A staircase of flat steps — each leaf predicts the AVERAGE price of its training houses", "A smooth straight line through the data", "A smooth curve like polynomial fit", "A jagged line through every training point", "A single flat line at the overall mean"],
...

{
  ...
  choices: ["It predicts a flat constant — roughly its 2020 leaf — because trees cannot extrapolate beyond the training range", "It continues the 10% yearly growth", "It refuses with an error", "It predicts zero for unseen years", "It interpolates between 2020 and infinity"],
...

{
  ...
  choices: ["Importances swing between retrains — especially with correlated features — so treat them as rough hints, not rankings", "The pipeline has a bug — importances are deterministic", "Income is definitely the top feature", "The average, 27%, is income's exact true importance", "More retrains will make the scores converge to one value"],
...

{
  ...
  choices: ["Dropping rows bleeds accuracy as data vanishes; native handling learns a default branch for missing values and degrades far slower", "Both approaches degrade identically", "Dropping rows is safer at every level", "Native handling fails above 10% missing", "Missing values make trees unusable either way"],
...

{
  ...
  choices: ["A leaf's probability is just the class fraction of its training samples — tiny leaves give 0/1 answers, so enforce bigger leaves or calibrate", "From a sigmoid applied to leaf depth", "From distances to the decision boundary", "The tree computes true Bayesian posteriors", "Extremes mean the model is exceptionally confident and well-calibrated"],
...

{
  ...
  choices: ["Deep trees are unstable: tiny data changes rewrite them — exactly the variance that bagging and forests average away", "Trees converge to identical predictions as depth grows", "Shallow trees disagree more than deep ones", "Disagreement proves one tree is broken", "Depth affects speed but not stability"],
...

{
  ...
  choices: ["Each fraud sample counts 20× in the impurity maths, so splits that isolate fraud become worth making — recall rises at precision's expense", "It duplicates fraud rows 20 times on disk", "It changes the decision threshold at predict time", "It caps the majority class at 20 samples per leaf", "It switches the criterion from gini to entropy"],
...

{
  ...
  choices: ["Strictly binary splits, gini by default, and native support for numeric features and regression", "Multiway splits on every categorical level", "It only handles categorical features", "It cannot be pruned", "It requires feature scaling first"],
...

{
  ...
  choices: ["Each one-hot column can only split one level off at a time, so useful groupings need dozens of splits and each column looks weak at the fork", "Trees cannot process 0/1 features at all", "One-hot causes the zero-frequency problem", "It makes gini impossible to compute", "Because one-hot requires feature scaling"],
