...

{
  ...
  choices: ["It forbids leaves smaller than N samples, so the tree can't memorise single points — validation peaks at a middle value", "It caps the tree's depth at N levels directly, so no branch can grow too deep, and validation therefore peaks at the smallest N", "It restricts how many features each individual split may consider, so validation just keeps climbing as N grows larger", "It strips the N most extreme outliers out of the data well before training, so noisy points can never form leaves", "It sets the minimum accuracy that every leaf must reach, forcing impure leaves to keep on splitting until they comply"],
...

{
  ...
  choices: ["It predicts a flat constant — roughly its 2020 leaf — because trees cannot extrapolate beyond the training range", "It carries the observed 10% yearly growth forward, extending the fitted upward trend cleanly out to the 2024 value and beyond", "It refuses outright and raises a runtime error, since 2024 lies well outside every single leaf it ever learned", "It predicts a value of exactly zero for 2024, because that specific year never once appeared in the training data", "It interpolates smoothly between the 2020 leaf and the very largest sales value it ever saw anywhere during training"],
...

{
  ...
  choices: ["A leaf's probability is just the class fraction of its training samples — tiny leaves give 0/1 answers, so enforce bigger leaves or calibrate", "The probability comes from a sigmoid function applied to the depth of the landed leaf, so shallow leaves report the confident extremes", "It comes from each point's signed distance to the learned decision boundary, and points sitting far away get pushed toward 0 or 100%", "The tree computes a genuine Bayesian posterior for each class, so the extreme 0/100 outputs are exact probabilities rather than mere counting artefacts", "Extremes simply mean the model is unusually confident and already well-calibrated, so 0/100 answers need no further correction"],
...

{
  ...
  choices: ["Deep trees are unstable: tiny data changes rewrite them — exactly the variance that bagging and forests average away", "Deep trees converge to identical predictions as depth grows, since the added splits leave ever less room to disagree", "Shallow trees actually disagree far more than deep ones do, since a single early split decides almost their whole verdict", "Any disagreement between the two just proves that one of the trees was simply trained or configured incorrectly somewhere", "Depth affects only prediction speed, never stability, so the two trees still track each other closely at every depth"],
...

{
  ...
  choices: ["Each fraud sample counts 20× in the impurity maths, so splits that isolate fraud become worth making — recall rises at precision's expense", "It physically duplicates every fraud row twenty times across the dataset before training begins, so the stored file grows twentyfold on disk", "It shifts the decision threshold only at predict time, leaving the trained tree itself completely unchanged underneath it", "It caps the majority class at just twenty samples per leaf, forcing the tree to make room for the far rarer fraud cases", "It quietly switches the split criterion from gini over to entropy, which just happens to favour the minority class more"],
...

{
  ...
  choices: ["Each one-hot column can only split one level off at a time, so useful groupings need dozens of splits and each column looks weak at the fork", "Decision trees simply cannot process 0/1 indicator columns at all, so one-hot encoding must always be undone before any training", "One-hot encoding triggers the classic zero-frequency problem, so any unseen combination of states is assigned a predicted probability of exactly zero", "Spreading a feature across fifty separate columns makes the gini impurity impossible to compute at any split on that feature", "One-hot columns must be feature-scaled first, and trees behave rather badly whenever their raw inputs are left unscaled"],
