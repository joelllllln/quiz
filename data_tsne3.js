...

{
  ...
  choices: ["The 'crowding problem': a high-D neighbourhood has far more room than 2-D, so mapping it faithfully crushes points together — the t-distribution's heavy tail gives moderately-distant points more space, letting clusters spread out instead of collapsing", "The 'convergence problem': the heavy tail smooths the loss surface so gradient descent settles far faster and never stalls in the poor local minima where a Gaussian low-D kernel would otherwise repeatedly trap the whole optimisation on large high-dimensional datasets", "The 'tuning problem': the heavy tail fixes an effective neighbourhood size on its own, so it removes any need to set a perplexity parameter and lets the map choose its own scale fully automatically without a sweep", "The 'shape problem': the heavy tail forces every emerging cluster to come out as a clean, roughly circular blob, guaranteeing perfectly round islands instead of the ragged, stringy shapes a Gaussian low-D kernel gives", "The 'scale problem': heavy-tailed similarities are far cheaper to store and compute than Gaussian ones, cutting the memory and time cost of the pairwise-similarity matrix without changing the resulting picture at all"],
...

{
  ...
  choices: ["It minimises KL(high-D ‖ low-D), which is ASYMMETRIC: it heavily punishes putting true neighbours far apart but barely punishes putting non-neighbours close — so it preserves LOCAL structure and is free to distort GLOBAL structure", "It minimises plain squared error between the high-D and low-D pairwise distances, a symmetric and unbiased objective that treats neighbours and strangers exactly alike and therefore preserves both the local and the global geometry equally well", "It maximises the retained variance exactly as PCA does, only in a non-linear fashion, so like PCA its axes and its between-cluster distances both stay fully faithful to the original data", "The divergence it optimises is perfectly symmetric, penalising torn-apart neighbours and stuck-together strangers equally, so the resulting map carries no systematic local-versus-global bias whatsoever", "It directly optimises classification accuracy against labels hidden from the embedding, which is exactly why its islands line up so tightly with the true underlying categories in the data"],
...

{
  ...
  choices: ["t-SNE has no reusable 'model' — it optimises all point positions jointly from scratch, so a new point would require re-running everything; UMAP learns a reusable mapping (fuzzy-topological structure) it can apply via .transform() to unseen points", "t-SNE is merely far slower at this, not actually incapable of it, and given enough compute it could re-embed each new streaming point onto the frozen existing map in real time, exactly the way UMAP's transform does", "UMAP simply stores every single training point and then, for each new arrival, looks up its nearest stored neighbour and copies that neighbour's existing map coordinates directly onto the newcomer, which only imitates a real transform without learning one", "t-SNE positions each of its points using class labels, and streaming data always arrives unlabelled, whereas UMAP is fully unsupervised and can therefore place unlabelled newcomers onto the existing map without any trouble", "New points can indeed be added, but only in fixed batches of about a thousand at a time, because t-SNE re-optimises the entire layout one whole block at once rather than a single point at a time"],
...

{
  ...
  choices: ["PCA: linear, fast, reversible, for compression/denoising/preprocessing (features for models). t-SNE: non-linear, exploratory VISUALISATION of local structure, non-parametric. UMAP: non-linear, faster, better global layout, reusable transform — visualisation AND general reduction", "They are effectively interchangeable general-purpose reducers: all three retain both local and global structure about equally well, and all three can transform new data, so you may simply pick whichever one you happen to know best by habit", "Always reach straight for UMAP, because it strictly dominates the other two on every single axis at once: it is faster than both of them, more accurate than t-SNE at local structure, and more faithful, reversible, and interpretable than PCA on essentially any real dataset you will ever meet", "Choose strictly by the data type in front of you: PCA is the correct tool for image data, t-SNE is specifically designed for text and language data, and UMAP is meant only for ordinary tabular data with named numeric columns, and mixing them up is a real mistake", "Use t-SNE for essentially everything, since it reveals by far the richest and most detailed structure, and treat both PCA and UMAP as older legacy methods now kept around only for backward compatibility with existing pipelines"],
