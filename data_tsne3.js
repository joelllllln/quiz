...

{
  ...
  choices: ["The 'crowding problem': a high-D neighbourhood has far more room than 2-D, so mapping it faithfully crushes points together — the t-distribution's heavy tail gives moderately-distant points more space, letting clusters spread out instead of collapsing", "The 'convergence problem': the heavy tail smooths the loss surface so gradient descent settles far faster and never stalls in the poor local minima where a Gaussian low-D kernel would otherwise repeatedly trap the whole optimisation on large datasets", "The 'tuning problem': the heavy tail fixes an effective neighbourhood size on its own, so it removes any need to set a perplexity parameter and lets the map choose its own scale fully automatically without a sweep", "The 'shape problem': the heavy tail forces every emerging cluster to come out as a clean, roughly circular blob, guaranteeing perfectly round islands instead of the ragged, stringy shapes a Gaussian low-D kernel gives", "The 'scale problem': heavy-tailed similarities are far cheaper to store and compute than Gaussian ones, cutting the memory and time cost of the pairwise-similarity matrix without changing the resulting picture at all"],
