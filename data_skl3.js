...

{
  ...
  choices: ["A class with fit(X, y=None) that LEARNS and stores state (the percentiles) returning self, and transform(X) that APPLIES it — inherit BaseEstimator + TransformerMixin for get_params/set_params and fit_transform for free", "A plain function applied to the whole dataset before splitting", "Override Pipeline's internals with a lambda", "Subclass GridSearchCV and inject the step", "Custom steps can't participate in Pipelines"],
...

{
  ...
  choices: ["Named-step parameter paths like 'scaler': [StandardScaler(), RobustScaler()] and 'pca': [PCA(20), 'passthrough'] in the grid — steps are parameters too, and 'passthrough' deletes a step", "Nested for-loops around multiple GridSearchCVs", "It can't; structure is fixed before searching", "A separate library called PipelineSearch", "Editing pipeline.steps between fits"],
...

{
  ...
  choices: ["Pickles are snapshots of code+state tied to library versions — pin versions and store metadata for the short term; for longevity or cross-platform serving, export to a neutral format like ONNX, and always keep the training pipeline reproducible", "joblib files are guaranteed stable across versions", "Retrain from scratch at every deploy; persistence is an anti-pattern", "Save model.get_params() as JSON — that's the whole model", "Store the Python source code alongside; loading then always works"],
...

{
  ...
  choices: ["Read data in chunks and call partial_fit(chunk) repeatedly — supported by SGD models, MultinomialNB, MiniBatchKMeans, IncrementalPCA etc.; pair with stateless/hashing feature steps so preprocessing streams too", "Set n_jobs=-1 and fit as usual", "Convert to sparse and fit normally — sparsity always fits memory", "Random-sample 1M rows; more adds nothing", "Buy RAM; sklearn has no out-of-core story"],
...

{
  ...
  choices: ["get_feature_names_out() propagates names through every step, and set_output(transform='pandas') makes transformers emit named DataFrames — so downstream importances map straight back to 'city=Paris', not 'x412'", "Manually track column indices in a spreadsheet", "Avoid one-hot encoding so names never change", "Names are cosmetic; use indices in reports", "Rename columns after training by guessing the order"],
