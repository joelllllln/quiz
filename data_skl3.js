...

{
  ...
  choices: ["Named-step parameter paths like 'scaler': [StandardScaler(), RobustScaler()] and 'pca': [PCA(20), 'passthrough'] in the grid — steps are parameters too, and 'passthrough' deletes a step", "Prefix each value with the step name, like 'scaler__use': [True, False] and 'pca__enabled': [1, 0] — every step secretly exposes a hidden boolean flag that GridSearchCV simply flips to include or drop the step", "Pass structure='search' to GridSearchCV together with a list of candidate Pipelines, and it cross-validates each full architecture and returns the single highest-scoring one", "Set the grid value to 'drop' for any step and 'auto' for the estimator slot, letting GridSearchCV assemble the pipeline layout itself from whatever steps remain", "Use RandomizedSearchCV instead, since only random search can sample over whole estimator objects rather than plain numeric hyperparameter values"],
