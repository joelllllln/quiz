...

{
  ...
  choices: ["Layer 1 captures most of the gain; each further layer adds noise-level improvements while multiplying leakage risk, compute and fragility — competitions tolerate that, production almost never should", "Each added layer compounds the gain of the layer directly beneath it, so a three-layer tower reliably multiplies the single-meta dividend — the extra fragility is real but the accuracy return scales right alongside it", "Deeper stacks average away the top layer's variance, so each level lowers overfitting automatically and the private-leaderboard gap steadily shrinks as you add more layers", "Roughly three layers is the proven sweet spot across domains; below it you leave gains unclaimed, above it the out-of-fold folds simply run out of rows left to split", "Past one meta-layer the construction is mathematically degenerate — the second meta-model can only reproduce the first, so extra layers are provably impossible to fit at all"],
...

{
  ...
  choices: ["When trust in base models should be CONTEXT-DEPENDENT — raw features let the meta-model learn 'believe the forest on segment X, the linear model elsewhere', not just fixed global weights", "When the base models are poorly calibrated — the raw features let the meta-model recalibrate each one's probabilities before deciding how much weight to assign", "When you have very few training rows — the extra raw columns regularise the meta-model and stop it overfitting the handful of prediction features", "When the base models tend to agree — raw features give the meta-model a tie-breaker so it can pick a winner instead of averaging identical predictions", "When you want to skip the out-of-fold step — passing the raw features lets the meta-model reconstruct each base model's honest behaviour without paying for the cross-validation folds at all"],
...

{
  ...
  choices: ["Shuffled folds let base models train on the FUTURE of the rows they predict — the meta-model then learns to trust base models at an accuracy they'll never have in production; use walk-forward out-of-fold generation", "The real leak is feature scaling: standardising each column over the entire series before splitting lets every fold quietly absorb future statistics, so the fix is to normalise inside each fold rather than change the split", "Five folds is simply too few for temporal data — each holdout block spans too long a stretch, so keep raising the fold count until every block covers just a single time period", "The meta-model itself leaked: fitted on all oof rows at once it glimpsed future columns; refit it strictly fold-by-fold and the shuffled base predictions become perfectly safe to stack on", "Shuffling the folds is harmless; the genuine leak is the final refit on the whole series, which lets the shipped base models quietly memorise the future window before the meta-model grades them"],
...

{
  ...
  choices: ["Averaging treats every stated probability as equally honest, so the loudest (most extreme) member dominates every vote regardless of skill — calibrate each member first, THEN average", "Averaging quietly rewards the under-confident forest, whose timid mid-range 0.7s drag every vote back toward 0.5 and drown out the decisive members — sharpen its probabilities first, then average", "The three probability scales don't share a common range, so the mean is meaningless; min-max rescale each member's outputs to 0-1 before averaging them together", "Soft voting silently drops the calibrated member because its probabilities sit nearest 0.5 and add least to the average — up-weight the humble members to fix it", "Averaging cancels each model's calibration error against the others', so nothing breaks; the real fix is simply adding more members until the biases wash out"],
...

{
  ...
  choices: ["Train ONE student model to mimic the stack's PROBABILITY outputs (soft targets) on abundant unlabelled data — the soft labels carry the ensemble's learned dark knowledge, and the student often lands within a fraction of the teacher", "Train the student only on the stack's hard 0/1 decisions across a huge labelled training set — clean majority-vote targets transfer the committee's knowledge far more reliably than fuzzy, noisy probability outputs, and fit any fast model", "Average the twelve fitted models' weights into a single network of identical shape, so that one forward pass reproduces the whole committee's function at roughly a twelfth of the original serving cost", "Prune the library down to its three most accurate members and refit a small meta-model on only those, which recovers most of the ensemble's edge while slipping inside the latency budget", "Have one student learn to predict which base model the stack leaned on for each row, then invoke only that single chosen model at inference time to stay under the tight memory and latency ceiling"],
...

{
  ...
  choices: ["Greedy selection with replacement IS coarse weight learning (counts = weights) but searches a tiny discrete space, so it barely overfits the validation set — and it prunes harmful models to exact zero automatically", "Refitting continuous weights only needs gradients and a convex loss, while the greedy loop needs neither, so the loop is really the same weight optimisation reached by a slower, gradient-free route over one identical search space", "With a library of 200 models the greedy average is all but guaranteed to dominate any careful 10-model weighting, purely because piling on more members keeps cutting the ensemble's variance further", "Greedy addition quietly scores each candidate on the untouched test set rather than validation, so its picks generalise honestly while the continuous optimiser overfits the validation noise it trains on", "Adding models one at a time makes greedy selection a form of boosting: each pick fits the residual error the current ensemble still gets wrong, which the elegant weight fit can never do"],
