...

{
  ...
  choices: ["A boundary enclosing the normal data's region in (kernel) feature space — new points falling OUTSIDE are flagged as anomalies; nu sets the fraction allowed outside", "A boundary between the real normal recordings and a synthetic fake class the SVM invents by scattering random noise points, then flagging anything on the fake side", "The single mean recording of all the normal sounds, flagging any new recording that differs from that average by more than a set distance threshold", "Nothing usable, since an SVM fundamentally needs two labelled classes to define a margin, so with only normal examples the one-class training simply fails", "A compact lossy codec for the audio, flagging a recording whenever it cannot be compressed below the reconstruction-error threshold learned from normal"],
