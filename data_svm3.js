...

{
  ...
  choices: ["Points comfortably beyond the margin contribute EXACTLY zero loss — so the solution depends only on the few boundary-region points (the support vectors), giving sparse, boundary-focused models", "Because distant points give zero gradient, hinge loss provably converges faster than log loss on every dataset, no matter the size, the class balance, or the kernel choice - a guaranteed training-speed win", "It makes the SVM completely immune to outliers everywhere, because a point sitting far on the WRONG side also falls into a zero-loss region and is therefore ignored entirely by the optimiser while fitting", "The flat zone lets the SVM emit calibrated probabilities for free, since a point's depth into that zero-loss region can be read directly as its probability of belonging to the predicted class", "Nothing of real substance - hinge and log loss are merely two names for one identical objective, so the flat region changes nothing about the solution beyond a cosmetic difference in the scaling"],
...

{
  ...
  choices: ["Platt scaling: an internal cross-validated logistic regression is fitted to map SVM decision scores → probabilities — it costs extra training time and the calibration is approximate, so validate before betting on it", "The SVM natively produces true probabilities exactly like logistic regression, and probability=True simply switches on that built-in output - no extra model is fitted and no calibration step whatsoever runs at fit time at all", "The raw decision scores are simply min-max squashed onto the zero-to-one interval, so the smallest score maps to 0 and the largest to 1 - cheap, exact, and needing no held-out data or extra fitting at all", "At predict time it finds the two nearest support vectors to the query point and averages their class labels, turning that local average directly into the reported probability for the positive class", "predict_proba on SVC is a long-deprecated alias for predict that returns the same hard labels, so probability=True changes nothing meaningful and the numbers you see are not really probabilities at all"],
...

{
  ...
  choices: ["Kernel SVM cost grows steeply (superlinearly) with n — 45 SMALL two-class problems on ~1/5 of the data each train faster than 10 BIG all-data problems, and each subproblem stays cleanly separable", "One-vs-one is simply more accurate than one-vs-rest for every classifier ever built, so sklearn standardises on it across all of its estimators regardless of the underlying training-cost model or the data size", "The one-vs-rest scheme is covered by an active software patent that libsvm cannot license, so SVC is forced back onto the older one-vs-one tournament as the only unencumbered alternative that remains available", "One-vs-one duels can be trained with no kernel at all, whereas one-vs-rest strictly requires one, so SVC picks OvO purely to sidestep the expense of evaluating the kernel function during each fit", "It comes down to raw model count: 45 pairwise duels is simply fewer separate models to train and store than 10 one-vs-rest classifiers, so the tournament wins on sheer bookkeeping and memory footprint"],
...

{
  ...
  choices: ["Kernel approximation — map data through Nystroem/random Fourier features into an explicit feature space, then train a LINEAR SVM/SGD on it: near-kernel accuracy at linear-time cost", "There is genuinely no escape route - exact kernel SVMs are mandatory whenever you want kernel behaviour, so at 3M rows you simply have to budget the weeks of training time and buy the memory it demands", "Crush the data down to two dimensions with PCA first, then run an ordinary kernel SVC - with only two features the Gram matrix becomes small enough to fit in memory and train in linear time as usual", "Just swap the RBF kernel out for a low-degree polynomial one - polynomial kernels avoid building the Gram matrix entirely, so they train in linear time while keeping essentially the same accuracy", "Steadily raise the C parameter until training speeds up - a larger violation penalty prunes the support-vector set aggressively, collapsing the kernel computation down to near-linear cost at scale"],
...

{
  ...
  choices: ["A boundary enclosing the normal data's region in (kernel) feature space — new points falling OUTSIDE are flagged as anomalies; nu sets the fraction allowed outside", "A boundary between the real normal recordings and a synthetic fake class the SVM invents by scattering random noise points everywhere, then flagging anything on the fake side", "The single mean recording of all the normal sounds, flagging any new recording that differs from that overall average by more than a set distance threshold", "Nothing usable at all, since an SVM fundamentally needs two labelled classes to define a margin, so with only normal examples the one-class training simply fails outright", "A compact lossy codec for the audio, flagging a recording whenever it cannot be compressed below the reconstruction-error threshold learned from the normal recordings"],
...

{
  ...
  choices: ["They compensate each other along a diagonal ridge of good models — high-gamma-high-C corners memorise, low-low corners underfit, and the ridge means you tune them JOINTLY on log scales, never one at a time", "C and gamma act completely independently of one another, so the correct and efficient protocol is to fix one at a sensible default, sweep the other to its best value, then lock it and sweep the first", "In practice only C ever really matters - gamma has a negligible effect once features are scaled, so the heat-map is essentially flat along the gamma axis and you can safely leave gamma at its default", "The map reveals a hard rule: the best models always sit exactly where gamma equals 1/C, so rather than searching a grid you just set gamma to the reciprocal of your chosen C and skip the tuning entirely", "The landscape is essentially random noise that reshuffles with every random seed, so grid-searching it is futile - you should just accept the library defaults for C and gamma and move on to other things entirely"],
