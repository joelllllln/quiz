...

{
  ...
  choices: ["Each split can consult only ONE word, and evidence in text is spread thinly across thousands of words — a linear model sums all those tiny clues at once; a tree would need thousands of levels", "Decision trees cannot process columns that are overwhelmingly zeros, so the extreme sparsity of a 30,000-word matrix starves every candidate split of usable signal and the tree effectively never gets off the ground", "Text features have to be scaled to unit variance before a tree will work, and bag-of-words counts left raw make the impurity calculation wildly unreliable at every split", "The gini criterion is mathematically undefined for binary 0/1 features, so a tree grown on one-hot word columns falls back on far weaker heuristics to choose its splits", "Sparse matrices overwhelm tree implementations, which densify them internally, so 30,000 columns exhaust memory long before the tree can find the words that matter"],
...

{
  ...
  choices: ["For each candidate alpha, cross-validate a tree pruned at that alpha; pick the alpha with the best CV score (or the simplest within one SE) and refit on all training data", "Pick the candidate alpha that leaves the tree with the fewest leaves, since the whole point of pruning is to make the final tree as small and readable as it can possibly be", "Choose the largest alpha that still keeps training accuracy pinned at 100%, so you prune away only the branches that were never contributing to the fit", "Average all of the candidate alphas the path returns and then prune the fully grown tree exactly once at that single averaged value", "Skip the search entirely and set alpha to 0.01, the universal default that works acceptably across essentially every dataset"],
...

{
  ...
  choices: ["Monotonic constraints: declare income as monotone-increasing and the trainer only accepts splits whose predictions respect the direction — enforced by construction, not post-hoc patching", "Delete income from the model entirely: with the very feature that keeps violating the rule removed from the inputs, the ensemble simply can no longer produce a monotonicity breach on any segment", "Round income into clean bins of 10k before training, so the coarser feature can no longer wiggle up and down within a narrow slice and break the required direction", "Add a post-processing layer that detects any violating prediction and quietly flips it back into line, guaranteeing a monotone output without retraining", "Retrain the ensemble repeatedly with fresh random seeds until you land on a run whose splits happen, by luck, to respect the income direction everywhere"],
...

{
  ...
  choices: ["Random folds put FUTURE days in training while 'predicting' past days — and rolling features straddle fold boundaries — so CV rehearsed with tomorrow's newspaper; use time-based splits (TimeSeriesSplit)", "The tree was simply too shallow to capture temporal structure, so raising its maximum depth and adding a handful more lag and rolling features would have closed almost the entire gap between the two scores", "The 94% was little more than a rounding artefact of a tiny validation set, and a larger held-out sample would have reported something much closer to the real 71%", "Sales that move with time cannot be modelled by trees at all, since their flat leaf predictions can never track a series that keeps trending upward year on year", "The rolling averages were never scaled to a common range, and a tree fed unscaled window features silently overfits them, inflating the CV score"],
...

{
  ...
  choices: ["Trees: thresholds, interactions, mixed/messy tabular data, no scaling worries. Linear: wide sparse data, smooth additive trends, extrapolation, small samples, and when coefficients must be explained", "Always reach for trees: across essentially every modern tabular benchmark the boosted-tree ensembles win so consistently and by such margins that fitting a linear model is rarely worth the bother anymore", "Always reach for linear models: trees are essentially a legacy technique, and a well-regularised linear fit matches them on any dataset that actually matters in practice", "Use trees whenever the task is classification and linear models whenever it is regression, since each family was designed around one of those two problem types", "Just flip a coin between them: on average across datasets their accuracy comes out identical, so the choice only ever affects training speed, never quality"],
