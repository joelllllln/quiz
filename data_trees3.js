...

{
  ...
  choices: ["Each split can consult only ONE word, and evidence in text is spread thinly across thousands of words — a linear model sums all those tiny clues at once; a tree would need thousands of levels", "Trees cannot process zeros", "Text features must be scaled first", "The gini criterion is undefined for binary features", "Sparse matrices crash tree implementations"],
...

{
  ...
  choices: ["For each candidate alpha, cross-validate a tree pruned at that alpha; pick the alpha with the best CV score (or the simplest within one SE) and refit on all training data", "Pick the alpha that gives the fewest leaves", "Pick the largest alpha that keeps training accuracy at 100%", "Average all candidate alphas", "Pick alpha = 0.01, the universal default"],
...

{
  ...
  choices: ["The RATIO debt/income — a tree can only cut axis-by-axis, so a diagonal rule costs dozens of splits, while the precomputed ratio makes it ONE split", "A duplicate of the income column", "Debt squared", "A random noise column as regulariser", "The sum debt+income"],
...

{
  ...
  choices: ["Random folds put FUTURE days in training while 'predicting' past days — and rolling features straddle fold boundaries — so CV rehearsed with tomorrow's newspaper; use time-based splits (TimeSeriesSplit)", "The tree was too shallow for time series", "94% was a rounding error", "Sales data cannot be modelled with trees", "The rolling averages needed scaling"],
...

{
  ...
  choices: ["Trees: thresholds, interactions, mixed/messy tabular data, no scaling worries. Linear: wide sparse data, smooth additive trends, extrapolation, small samples, and when coefficients must be explained", "Trees always — they won the benchmarks", "Linear always — trees are legacy", "Trees for classification, linear for regression", "Flip a coin; performance is identical on average"],
