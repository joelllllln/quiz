...

{
  ...
  choices: ["ROC's false-positive RATE divides by the ocean of negatives, so thousands of false alarms barely move it — precision divides by YOUR ALERTS, where those same false alarms dominate", "PR curves are drawn upside down", "ROC-AUC is miscomputed under imbalance", "The model is actually fine and PR is pessimistic", "Precision and FPR are the same number scaled"],
...

{
  ...
  choices: ["Micro pools all decisions (majority classes dominate), macro averages per-class scores EQUALLY (rare classes count full weight), weighted sits between — macro is the rare-class watchdog", "They should be identical; the run is buggy", "Macro is micro rounded down", "Weighted always exceeds micro", "The disagreement means F1 is invalid for multiclass"],
...

{
  ...
  choices: ["Log-loss and the Brier score — proper scoring rules, minimised only by reporting your true belief, so they reward calibrated confidence and punish confident errors savagely", "RMSE on the labels", "Accuracy at five thresholds averaged", "AUC, which measures calibration directly", "No metric can evaluate probabilities without more labels"],
...

{
  ...
  choices: ["Bin predictions by stated confidence; per bin, plot stated vs ACTUAL frequency (curve below diagonal = overconfident) — ECE is the traffic-weighted average gap between the two", "Plot accuracy against training epochs", "Sort predictions and plot cumulative recall", "They require retraining with a special loss", "ECE is the area under the ROC curve"],
