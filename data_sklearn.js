/* Advanced Scikit-learn — Part I: Foundations. choices[0] is always correct (shuffled at render). */
...

{
  ...
  choices: ["The scaler is re-fitted inside each training fold — leakage becomes impossible by construction", "It runs twice as fast", "It removes the need for a test set", "It tunes hyperparameters automatically", "It converts data to NumPy"],
  ...
  widget: {
    type: "foldPick", title: "Discipline vs structure",
    ...
    extreme: { at: 5 },
    reveal: { name: "Pipeline", formula: "make_pipeline(StandardScaler(), SVC()) → cross_val_score(pipe, X, y)",
      text: "sklearn's most important convenience isn't convenience — it's correctness infrastructure. Pipelines make the leakage rules physically unbreakable." }
  }
},

{
  ...
  choices: ["ColumnTransformer", "StandardScaler with a flag", "A second Pipeline", "LabelEncoder on everything", "np.hstack after manual prep"],
  ...
  widget: {
    type: "scaleFeature", title: "Why the routing matters",
    ...
    aName: "one-hot: is_member (×10)", bName: "spend (raw £)",
    ...
    extreme: { at: "max" },
    reveal: { name: "ColumnTransformer", formula: "route column subsets to different transformers, concatenate outputs, Pipeline-safe",
      text: "The standard front door for real-world tabular data. With Pipeline, it forms the skeleton of virtually every production sklearn model." }
  }
},

{
  ...
  choices: ["Sample random combinations — near-best results from a small, fixed budget of fits", "Skip cross-validation to save time", "Search only the first two parameters", "A coarser grid with the same coverage", "Parallel fits with no other change"],
  ...
  widget: {
    type: "curveStatic", title: "Excellence is a region, not a pixel",
    ...
    xlab: "fits spent", xs: [0,1,2,3,4,5], labels: ["10","30","100","300","1k","15k"], dec: 1, yunit: "%",
    series: [
    ...
    extreme: { at: 2 },
    reveal: { name: "RandomizedSearchCV", formula: "sample n_iter combos from distributions — budget-controlled, near-optimal",
      text: "For more than 2–3 hyperparameters, random beats grid at any realistic budget. Bergstra & Bengio proved what practitioners had noticed." }
  }
},

{
  q: "cross_val_score(model, X, y, cv=5) returns [0.84, 0.79, 0.86, 0.81, 0.85]. What's the professional way to report this?",
  choices: ["Mean ± spread: 0.83 ± 0.03 — the spread is part of the result", "Just the best fold: 0.86", "Just the worst fold: 0.79, to be safe", "The first fold: 0.84", "The median times the mean"],
  ...
  widget: {
    type: "foldPick", title: "Five measurements, one claim",
    ...
    extreme: { at: 6 },
    reveal: { name: "Reporting CV results", formula: "scores.mean() ± scores.std() — compare models with the spread in view",
      text: "The ± is not decoration: it's the difference between detecting improvements and hallucinating them." }
  }
},

{
  q: "train_test_split(X, y, test_size=0.2, stratify=y) — what does the stratify argument guarantee?",
  choices: ["Both splits keep the original class proportions — vital when a class is rare", "The split is the same on every run", "The test set gets the hardest rows", "Rows are sorted before splitting", "Twenty percent of each FEATURE is held out"],
  ...
  widget: {
    type: "foldPick", title: "The lottery vs the fair deal",
    ...
    extreme: { at: 5 },
    reveal: { name: "Stratified splitting", formula: "train_test_split(..., stratify=y) · StratifiedKFold (sklearn's CV default for classifiers)",
      text: "One keyword between you and evaluation roulette. The rarer your positive class, the less optional it is." }
  }
},

{
  ...
  choices: ["The search optimises whatever you measure — different yardsticks crown different champions", "One of the searches must be buggy", "Recall and accuracy are the same for tuned models", "The random seed changed", "scoring only changes the printout"],
  ...
  widget: {
    type: "threshold", title: "The yardstick picks the champion",
    ...
    posName: "fraud", negName: "legit", axis: "model score", show: ["accuracy", "recall"],
    items: [{s:0.8,c:0},{s:1.6,c:0},{s:2.4,c:0},{s:3.2,c:0},{s:4,c:0},{s:4.8,c:0},{s:5.6,c:0},{s:6.4,c:1},{s:7.2,c:0},{s:8,c:1},{s:8.8,c:0},{s:9.6,c:1}],
    ...
    extreme: { at: "max" },
    reveal: { name: "The scoring parameter", formula: "scoring='recall' / 'f1' / 'roc_auc' / make_scorer(...) — the objective, made explicit",
      text: "Every optimiser optimises something. sklearn lets one keyword make sure it's the thing your problem actually rewards." }
  }
},

{
  q: "Some sklearn classifiers offer predict_proba, others decision_function, some both. What's the difference?",
  choices: ["predict_proba returns calibrated-ish probabilities (0–1); decision_function returns raw unbounded scores", "They're aliases for the same output", "decision_function is only for regression", "predict_proba is deprecated", "decision_function returns class labels"],
  ...
  widget: {
    type: "sigmoid", title: "Native units vs percentages",
    ...
    classes: ["negative", "positive"], xlab: "decision_function score (shifted)", b: 5, qx: 6.8,
    points: [{x:1,c:0},{x:2,c:0},{x:3,c:0},{x:4,c:0},{x:4.6,c:0},{x:5.6,c:1},{x:6.4,c:1},{x:7.4,c:1},{x:8.4,c:1},{x:9.2,c:1}],
    ...
    extreme: { at: "max" },
    reveal: { name: "predict_proba vs decision_function", formula: "raw score ∈ (−∞,∞) —sigmoid/calibration→ probability ∈ (0,1)",
      text: "Know which language each estimator speaks. Ranking is bilingual; thresholds by cost demand probabilities." }
  }
},

{
  ...
  choices: ["CalibratedClassifierCV — learn a mapping from claimed to actual probabilities on held-out folds", "GridSearchCV with more iterations", "StandardScaler on the outputs", "A lower decision threshold", "class_weight='balanced'"],
  ...
  widget: {
    type: "curveStatic", title: "The correction chart",
    ...
    extreme: { at: "max" },
    reveal: { name: "CalibratedClassifierCV", formula: "wrap fitted model → fit sigmoid/isotonic on out-of-fold predictions",
      text: "Accuracy checks the verdicts; calibration checks the confidence. Systems that ACT on probabilities need both audited." }
  }
},

{
  ...
  choices: ["Permutation — impurity importance over-credits high-cardinality columns the model memorised", "feature_importances_ — it's built into the model", "Neither — importance can't be measured", "Both — average them", "Whichever ranks income first"],
  ...
  widget: {
    type: "curveStatic", title: "Cross-examining the witness",
    ...
    xlab: "feature", xs: [0,1,2,3,4], labels: ["income","tenure","user_id_hash","age","random noise"], dec: 0, yunit: "%",
    series: [
    ...
    extreme: { at: 2 },
    reveal: { name: "permutation_importance", formula: "shuffle one column on validation data → importance = score drop",
      text: "Model-agnostic, leakage-revealing, honest. The built-in attribute is a hint; permutation is the audit." }
  }
},

{
  ...
  choices: ["Hard uses labels only; soft adds probabilities; stacking LEARNS the combination — most informed", "Hard uses the most information of the three", "They use identical information differently named", "Soft voting ignores the base models", "Stacking uses labels only"],
  ...
  widget: {
    type: "curveStatic", title: "Three committee charters",
    world: "The same three base models combined three ways, next to their best member. Slide up the ladder of information use.",
    xlab: "combination", xs: [0,1,2,3], labels: ["best single model","hard voting","soft voting","stacking"], dec: 1, yunit: "%",
    series: [
    ...
    extreme: { at: "max" },
    reveal: { name: "Voting vs stacking in sklearn", formula: "VotingClassifier(hard) < (soft) < StackingClassifier — rising information use",
      text: "The course's ideas, shipped as three classes: majority votes, probability averages, learned meta-models. You now know exactly what each line of that import buys." }
  }
}
];
