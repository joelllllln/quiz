...

{
  ...
  choices: ["It inflates distances in sparse regions (via each point's core-distance), so a single density threshold no longer shreds sparse clusters while merging dense ones — clusters at DIFFERENT densities coexist", "It makes all clusters spherical", "It removes the need for a distance metric", "It guarantees exactly k clusters", "It speeds DBSCAN up with no quality change"],
...

{
  ...
  choices: ["Distances concentrate — nearly all point-pairs sit at similar distances — so the k-distance plot flattens into no elbow, and any eps either captures everyone or no one: density stops being locally meaningful", "DBSCAN's code has a 20-dimension hard limit", "eps must be negative in high dimensions", "The clusters become too fast to compute", "High dimensions make every point a core point by definition"],
...

{
  ...
  choices: ["Use the haversine metric (great-circle distance on a sphere) with eps in radians — Euclidean treats lat/long as a flat grid, which warps badly away from the equator", "Scale latitude and longitude to [0,1]", "Switch to Manhattan distance", "Project to 3-D and use Euclidean", "Increase min_samples until it looks right"],
...

{
  ...
  choices: ["Embedding the graph into a few dimensions using the eigenvectors of its Laplacian (which encode connectivity), then running k-means there — so 'clusters' become well-CONNECTED groups, not spatially compact ones", "Running DBSCAN on the adjacency matrix directly", "Computing shortest paths between all nodes", "It's k-means with a different random seed", "Applying PCA to the raw customer features"],
