/* Stacking & Voting — Part I: Foundations. choices[0] is always correct (shuffled at render). */
...

{
  ...
  choices: ["Hard voting counts labels (class 0 wins 2–1); soft voting averages probabilities, so A's 90% overturns two lukewarm opinions", "Both pick class 0", "Both pick class 1", "Soft voting picks whichever model is most accurate", "Hard voting averages the probabilities"],
  ...
  widget: {
    type: "curveStatic", title: "A shrug vs a pounding fist",
    ...
    xlab: "model A's confidence in class 1 →", xs: [0,1,2,3,4], labels: ["55%","65%","75%","85%","95%"], dec: 0, yunit: "%",
    series: [
    ...
    extreme: { at: "max" },
    reveal: { name: "Hard vs soft voting", formula: "hard: argmax of label counts · soft: argmax of averaged probabilities",
      text: "sklearn: VotingClassifier(voting='hard'|'soft'). Prefer soft with calibrated members; stacking goes one further and LEARNS how much to trust each member." }
  }
},

{
  ...
  choices: ["Different model families — a tree, a linear model and a kNN err in structurally different ways; reseeding one model barely helps", "Different random seeds of the same model", "Training the same model twice as long", "Renaming the features", "Averaging the same model's output twice"],
  ...
  widget: {
    type: "curveStatic", title: "Five clones vs three professions",
    ...
    xlab: "how the ensemble is assembled →", xs: [0,1,2,3,4], labels: ["1 model","3 seeds","3 same-family","3 families","5 families"], dec: 0, yunit: "%",
    series: [
    ...
    extreme: { at: "max" },
    reveal: { name: "Ensemble diversity", formula: "gain ∝ member accuracy × (1 − error correlation)",
      text: "Sources of diversity, strongest first: different families > different features/data views > different hyperparameters > different seeds. Stacking exploits exactly this." }
  }
},

{
  ...
  choices: ["Worth it when errors are expensive or you're competing; rarely worth 8× the serving cost for a routine business dashboard", "Always — accuracy is the only criterion", "Never — stacking gains are statistical noise", "Only when the ensemble also trains faster", "Only for image data"],
  ...
  widget: {
    type: "curveStatic", title: "What does a point cost?",
    ...
    xlab: "base models in the stack →", xs: [0,1,2,3,4], labels: ["1","4","8","16","32"], dec: 1, yunit: "",
    series: [
    ...
    extreme: { at: "max" },
    reveal: { name: "When stacking earns its keep", formula: "value of a point vs (serving cost + ops burden + opacity) × N",
      text: "Competitions and high-stakes screening: stack. Routine production: single strong model, or knowledge-distil the ensemble into it." }
  }
},

{
  ...
  choices: ["Blending predicts on one held-out slice (simple, wastes that data); stacking uses out-of-fold predictions covering ALL rows (thorough, more work)", "Blending uses hard labels, stacking uses probabilities", "Stacking has no meta-model", "Blending only works with two base models", "They are two names for exactly the same procedure"],
  ...
  widget: {
    type: "curveStatic", title: "One small exam vs a full rotation",
    ...
    xlab: "training rows →", xs: [0,1,2,3,4], labels: ["500","2k","10k","50k","200k"], dec: 1, yunit: "%",
    series: [
    ...
    extreme: { at: "max" },
    reveal: { name: "Blending vs stacking", formula: "blending: one holdout slice · stacking: K-fold out-of-fold, every row covered",
      text: "sklearn's StackingClassifier does the out-of-fold dance via cv=. Either way, the iron rule: the meta-model must never see a prediction made on that prediction's own training row." }
  }
}
];
