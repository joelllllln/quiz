/* Random Forests & Bagging — Part I: Foundations. choices[0] is always correct (shuffled at render). */
...

{
  ...
  choices: ["Tree diversity vs individual tree strength — fewer features per split decorrelates the trees, until they get too weak", "Training speed vs memory", "Depth vs number of trees", "Precision vs recall", "Bias of the bootstrap vs its variance"],
  ...
  widget: {
    type: "curveStatic", title: "A committee of clones is one person",
    ...
    xlab: "max_features per split →", xs: [0,1,2,3,4], labels: ["1","3 (sqrt)","5","7","8 (all)"], dec: 0, yunit: "%",
    series: [
    ...
    extreme: { at: "max" },
    reveal: { name: "max_features (the decorrelation knob)", formula: "ensemble error ↓ when trees are accurate AND uncorrelated",
      text: "Defaults: sqrt(n_features) for classification, ~n/3 or 1.0 for regression. If your forest barely beats one tree, correlation — not tree quality — is usually the culprit." }
  }
},

{
  ...
  choices: ["The split THRESHOLDS — cut points are drawn at random instead of optimised, making trees faster to grow and even less correlated", "The class labels during training", "The number of trees per prediction", "The loss function per tree", "The test set for each tree"],
  ...
  widget: {
    type: "curveStatic", title: "Sloppy questions, honest committee",
    ...
    xlab: "label noise in training data →", xs: [0,1,2,3,4], labels: ["none","5%","10%","20%","30%"], dec: 0, yunit: "%",
    series: [
    ...
    extreme: { at: "max" },
    reveal: { name: "ExtraTrees (extremely randomized trees)", formula: "random thresholds + (usually) no bootstrap → cheaper, less correlated trees",
      text: "sklearn: ExtraTreesClassifier/Regressor. Try it whenever you'd try a random forest — especially with noisy features or a training-time budget." }
  }
},

{
  ...
  choices: ["A much smoother curve — but still flat beyond the training range, because every tree still predicts leaf averages", "A perfectly smooth line with full extrapolation", "A staircase identical to one tree's", "A curve that oscillates wildly", "A straight line through the mean"],
  ...
  widget: {
    type: "curveStatic", title: "300 staircases and one shared shrug",
    ...
    extreme: { at: "max" },
    reveal: { name: "Forest regression = averaged staircases", formula: "smooth inside the training range · constant outside it",
      text: "Bagging fixes variance (jaggedness), never structural bias (no extrapolation). For trending targets, detrend first or use a model with a slope." }
  }
},

{
  ...
  choices: ["RF = robust with almost no tuning and parallel training; boosting = higher ceiling but needs careful tuning and overfits more easily", "Boosting is better in every respect", "RF is better in every respect", "RF for regression, boosting for classification", "They always produce identical results"],
  ...
  widget: {
    type: "curveStatic", title: "Committee vs relay",
    ...
    xlab: "criterion →", xs: [0,1,2,3,4], labels: ["works untuned","noise robustness","parallel training","peak accuracy","safe to overtrain"], dec: 0, yunit: "",
    series: [
    ...
    extreme: { at: "max" },
    reveal: { name: "RF vs gradient boosting", formula: "RF: variance ↓ via averaging · boosting: bias ↓ via sequential correction",
      text: "Start with RF for a robust baseline; reach for tuned boosting (XGBoost/LightGBM) when accuracy is worth the tuning budget. On tabular data these two remain the champions to beat." }
  }
}
];
