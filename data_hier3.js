...

{
  ...
  choices: ["It updates cluster-distances incrementally after each merge from the PREVIOUS distances alone — turning repeated full recomputation into O(1) updates — but Ward's version is valid only with Euclidean distance", "It lets Ward work with any distance metric", "It removes the need to choose k", "It makes single linkage behave like complete", "It only applies to datasets under 100 points"],
...

{
  ...
  choices: ["Original pairwise distances vs COPHENETIC distances (the merge height at which each pair first joins the same cluster) — low correlation means the tree is distorting the real geometry, so distrust its cuts", "Training vs validation accuracy of the clustering", "The distances before and after scaling", "Silhouette scores across two runs", "It measures how many clusters the tree has"],
...

{
  ...
  choices: ["A memory-bounded tree of Clustering Features (per-subcluster count + linear-sum + squared-sum) built in one streaming pass — it summarises dense regions into compact stats, trading exactness for O(n) single-pass scalability", "A decision tree that predicts cluster labels", "A compressed pickle of the full linkage matrix", "A GPU reimplementation of Ward linkage", "It stores every point but sorts them for speed"],
