...
(function () {
  ...
  card("interp",
    ...
    ["LIME is always strictly more accurate than SHAP on every kind of model, so SHAP is only ever worth reaching for on the very simplest linear models and never on tree-based ensembles",
  ...
  card("evalx",
    ...
    ["ROC-AUC simply cannot be computed at all whenever the two classes in the data happen to be meaningfully imbalanced against each other",
  ...
  card("evalx",
    ...
    ["They are really just three different names for one and the very same underlying formula, always computed in exactly the same way and therefore always giving you identical numbers on any dataset",
  ...
  card("valid",
    ...
    ["All three of these are really just three different names for the one exact same random shuffling procedure under the hood, and they each prevent precisely the same kinds of leakage as one another every time",
