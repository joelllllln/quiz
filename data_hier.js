/* Hierarchical Clustering — Part I: Foundations. choices[0] is always correct (shuffled at render). */
...

{
  ...
  choices: ["Merge the pair whose union INCREASES within-cluster variance the least — the same quantity k-means minimises", "Merge the pair with the closest two individual points", "Merge the pair with the closest centroids, ignoring spread", "Merge the two smallest clusters", "Merge the pair with the most similar sizes"],
  ...
  widget: {
    type: "curveStatic", title: "Two roads up the same hill",
    ...
    xlab: "k (clusters) →", xs: [0,1,2,3,4], labels: ["2","3","4","6","8"], dec: 0, yunit: "%",
    series: [
    ...
    extreme: { at: "max" },
    reveal: { name: "Ward linkage", formula: "merge the pair minimising Δ(within-cluster sum of squares)",
      text: "sklearn default, needs Euclidean distance. Compact round-ish clusters → Ward; chains/filaments → single; a robust middle ground → average linkage." }
  }
},

{
  ...
  choices: ["Top-down: start with everything in one cluster and recursively split — each split is its own expensive optimisation, so it's costlier and rarely implemented", "Bottom-up but with random merge order", "It only works on 1-D data", "It's another name for k-means", "It produces no dendrogram"],
  ...
  widget: {
    type: "curveStatic", title: "Merging is easy, splitting is hard",
    ...
    xlab: "points in play →", xs: [0,1,2,3,4], labels: ["4","8","12","16","20"], dec: 0, yunit: "",
    series: [
    ...
    extreme: { at: "max" },
    reveal: { name: "Divisive (top-down) clustering", formula: "2^(n−1)−1 binary splits vs n(n−1)/2 merge pairs",
      text: "Bisecting k-means (repeatedly 2-means-split the worst cluster) is the practical divisive method — sklearn ships it as BisectingKMeans. Everything else hierarchical is agglomerative in practice." }
  }
},

{
  ...
  choices: ["Cluster a sample (or k-means-compressed prototypes), build the tree on that, then assign the rest to the nearest cluster", "Buy more RAM until n² fits", "Run it on each half and concatenate the trees", "Sort the data first to make distances cheap", "Hierarchical clustering simply cannot be approximated"],
  ...
  widget: {
    type: "curveStatic", title: "The committee trick",
    ...
    xlab: "dataset size →", xs: [0,1,2,3,4], labels: ["5k","20k","100k","500k","2M"], dec: 0, yunit: "",
    series: [
    ...
    extreme: { at: "max" },
    reveal: { name: "Scaling hierarchical clustering", formula: "sample / compress to prototypes → tree on prototypes → nearest-assign the rest",
      text: "MiniBatchKMeans-to-1000-centroids then Ward on the centroids is a robust recipe. BIRCH does the compression trick natively (a CF-tree of summaries) and ships in sklearn." }
  }
},

{
  ...
  choices: ["Only clusters that are GRAPH-NEIGHBOURS may merge — clusters must grow through connected regions instead of jumping across gaps", "It only speeds things up; results are identical", "It fixes the number of final clusters", "It converts the method to divisive", "It replaces the distance metric with graph hops"],
  ...
  widget: {
    type: "curveStatic", title: "Grow through the material, not across the gap",
    ...
    xlab: "how curved the data's shape is →", xs: [0,1,2,3,4], labels: ["round blobs","ellipses","arcs","spirals","swiss-roll"], dec: 0, yunit: "%",
    series: [
    ...
    extreme: { at: "max" },
    reveal: { name: "Connectivity-constrained clustering", formula: "connectivity = kneighbors_graph(X, 10) → merges allowed only between graph-neighbours",
      text: "AgglomerativeClustering(connectivity=...). Use for manifold-shaped or geographic data; it also slashes runtime by shrinking the candidate-merge set." }
  }
},

{
  ...
  choices: ["Cut through the tallest vertical gap — the longest distance-range where no merges happen marks well-separated clusters", "Always cut to get exactly three clusters", "Cut at the average merge height", "Cut just above the first merge", "The dendrogram cannot inform the choice of k"],
  ...
  widget: {
    type: "dendro", title: "Cut through the silence",
    ...
    extreme: { at: "max" },
    reveal: { name: "Choosing k from the dendrogram", formula: "cut inside the tallest merge-free vertical gap",
      text: "Equivalent intuition to the elbow/silhouette: big gap = tight-inside, far-apart clusters. If no tall gap exists, the data may simply not have distinct clusters — also an answer." }
  }
}
];
